...
void feed_forward(Network *net , float *input , int n){
	...
	for(int i =1;i<NUM_LAYERS;i++){
		...
		mat_sig(net->layers[i].Z,net->layers[i].A);
	...
	for(int ii =0;ii<epochs;ii++){
		...
		for(int i =0;i<m;i++){
			...
			for(int j =0; j<net->layers[NUM_LAYERS-1].neurons;j++){
				...
				net->layers[NUM_LAYERS-1].delta->data[j][0] = 2.0f * (temp-output[i][j]) * (temp) * (1 - temp);
			}
			...
			for(int l = NUM_LAYERS-1; l>1; l--){
				...
				matrix *deriv = mat_alloc(net->layers[l-1].A->rows,net->layers[l-1].A->columns);
				mat_sig_deriv(net->layers[l-1].A,deriv);
